...
class SearchAlumnosController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($usuarioUpdate->count()==0 && $request->email!=''){
            ...
            $alumnoInsert = Alumno::insert(array(
            ...
            if($alumnoInsert){
                ...
                Mail::to($request->email)->send(new NewAlumnoMail($data));
            }
    ...
    {
        $response = "";
        $alumnosCheck= Alumno::where('id', $request->id)->get();
        if($alumnosCheck->count()>0){
            ...
            $alumnosUpdate = Alumno::where('id', $request->id)
                ->update(array(
                    'nombre' => $request->nombre,
                    'apellidoP' => $request->apellidoP,
                    'apellidoM' => $request->apellidoM,
            ...
            if($usuarioUpdate=="1" && $alumnosUpdate=="1"){
                $response = "Actualizado con exito!";
            }else{
                $response = "Fallo la actualización";
            }
        }else{
            $response = "No existe el alumno";
        }
    ...
    public function deleteAlumno(Request $request)
    {
        ...
        if($request->id){
            ...
            ->delete(); 
            $alumnoUpdate = Alumno::where('id', $request->id)
            ->delete(); 
            if($usuarioDelete){
    ...
    public function autosearch(Request $request)
    {
        ...
        if($term){
            $alumnos = Usuario::join('alumnos', 'usuario.id', '=', 'alumnos.id')
                ->where('alumnos.nombre', 'like', '%' . $term . '%')
                ->orWhere('alumnos.matricula', 'like', '%' . $term . '%')
                ->orWhere('usuario.correo', 'like', '%' . $term . '%')
                ->orderBy('status', 'desc')
                ->orderBy('alumnos.fecha_ingreso', 'desc')
                ->get();
            return view('vistas.subvistas_dashboard.control_escolar.bloques_view_table.view_table_alumnos_search', compact('alumnos'), compact('carreras'));
        }else{
    ...
    public function editModal(Request $request)
    {
        ...
        $alumnos = Usuario::join('alumnos', 'usuario.id', '=', 'alumnos.id')->where('usuario.id', $id_alumno)->get();
        $carreras = Carrera::get();
        return view('vistas.subvistas_dashboard.control_escolar.bloques_view_table.modal_edit_alumno', compact('alumnos'), compact('carreras'));
    }
    public function addModal()
    {
        $carreras = Carrera::get();
        return view('vistas.subvistas_dashboard.control_escolar.bloques_view_table.modal_add_alumno', compact('carreras'));
    } 
}
