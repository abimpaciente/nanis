<div id="bajaempleado" class="modal">
    <div class="modal-content">
      <h4>Baja</h4>
      <meta name="csrf-token-baja" content="{{ csrf_token() }}">
      <p class="flow-text">Dar de baja empleado?</p>
    </div>
    <div class="modal-footer">
        <a class="modal-close waves-effect waves-green btn-flat">Cancelar</a>
        <a class="modal-close waves-effect waves-green btn-flat" onclick="bajaEmpleado('{{route('empleados_change_status')}}', '0')">Dar baja</a>
    </div>
  </div>
  <script>
...
function downOutEmpleado(id){
  id_empleado = id;
}
function bajaEmpleado(url, status){
    if(url!='')
    {
      ...
      $.ajax({
        ...
        success: function(data)
        {
          ...
          ViewConfig('empleados', 'control-swipe-2')
          offLoad()
        },
