<div id="editcarrera" class="modal modal-fixed-footer">
    <div class="modal-header red darken-3 center-align" style="color:white;"><h4>Editar Carrera</h4></div>
    <div class="modal-content">
      <meta name="csrf-token-modal_carrera_edit" content="{{ csrf_token() }}">   
      <div class="row" id="content_edit_carrera_modal">             
          
      </div>
    </div>
    <div class="modal-footer">
        <a class="modal-close waves-effect waves-green btn-flat">Cancelar</a>
        <a class="modal-close waves-effect waves-green btn-flat" onclick="ActualizarCarrera('{{route('carrera_update')}}')">Editar</a>
    </div>
  </div>
      <style>
        #toast-container {
    ...
    // Or with jQuery
        var id_carrera_update = "";
      function UpdateCarrera(id_carrera){
        id_carrera_update = id_carrera;
        var data = new FormData();
        data.append('_token', $("meta[name='csrf-token-modal_carrera_edit']").attr("content"));
        data.append('id_carrera', id_carrera_update);
        $.ajax({
            url:'{{route('carrera_editmodal')}}',
            type:'POST',
            contentType:false,
            data:data,
            dataType: "html",
            ...
            success:function (data) {
                $('#content_edit_carrera_modal').html(data)
                offLoad()
            },
    ...
    function ActualizarCarrera(url){
    if(url!='')
    {
            
        var clave_carrera = $('#clave_carrera_update').val();
        var nombre_carrera = $('#nombre_carrera_update').val();
        var descripcion_carrera = $('#descripcion_carrera_update').val();
        var area = $('#area_update').val();
        var semestres = $('#semestres_update').val();
