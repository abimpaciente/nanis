<div id="bajaswitch" class="modal">
    <div class="modal-content">
      <h4>Baja</h4>
      <meta name="csrf-token-baja" content="{{ csrf_token() }}">
      <p class="flow-text">Apagar Switch?</p>
    </div>
    <div class="modal-footer">
        <a class="modal-close waves-effect waves-green btn-flat">Cancelar</a>
        <a class="modal-close waves-effect waves-green btn-flat" onclick="bajaSwitch('{{route('switch_change_status')}}', '0')">Apagar</a>
    </div>
  </div>
  <script>
...
function downOutSwitch(id){
  id_switch = id;
}
function bajaSwitch(url, status){
    if(url!='')
    {
