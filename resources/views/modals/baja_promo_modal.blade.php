<div id="bajapromo" class="modal">
    <div class="modal-content">
      <h4>Baja</h4>
      <meta name="csrf-token-baja" content="{{ csrf_token() }}">
      <p class="flow-text">Dar de baja promo?</p>
    </div>
    <div class="modal-footer">
        <a class="modal-close waves-effect waves-green btn-flat">Cancelar</a>
        <a class="modal-close waves-effect waves-green btn-flat" onclick="bajaPromo('{{route('promos_change_status')}}', '0', '{{$tipo}}')">Dar baja</a>
    </div>
  </div>
  <script>
...
function downOutPromo(id){
  id_promo = id;
}
function bajaPromo(url, status, tipo){
    if(url!='')
    {
